...
result = new BigNumber(sb.toString());
break;
}catch(Exception exc){
...
result = bn_calc(fn,s,op).divideBy(bn_calc(fn,op+1,e));
break;
case 11:
result = booleanToBigNumber(bn_calc(fn,s,op).equals(bn_calc(fn,op+1,e)));
break;
case 12:
result = booleanToBigNumber(!bn_calc(fn,s,op).equals(bn_calc(fn,op+1,e)));
break;
case 13:
result = booleanToBigNumber(!bn_calc(fn,s,op).smallThan(bn_calc(fn,op+1,e)));
break;
case 14:
result = booleanToBigNumber(!bn_calc(fn,s,op).bigThan(bn_calc(fn,op+1,e)));
break;
case 15:
result = booleanToBigNumber(bn_calc(fn,s,op).bigThan(bn_calc(fn,op+1,e)));
break;
...
break;
case 42:
	result=fact(bn_calc(fn,s,op));
	break;
case 47:
if(args.size()==3){
if(bn_calc(fn,op+1,e).equals(new BigNumber())){
result=(BigNumber)args.elementAt(0);
}else{
...
getNum(fn,cs,1);
sumnum++;}
result=new BigNumber(sumnum);
break;
case 61:
result=new BigNumber(args.size()-2);
break;
case 78:
...
if(o>=10000){
	result = (BigNumber)var_value.elementAt(o-10000);
	}else{
...
BigNumber bn_calc(int fn,int s,int e){
int i=calc(fn,s,e);
if(i<0){
throw new RuntimeException("  ");
}
return (BigNumber)num_stack.elementAt(i);
}
...
BigNumber getNum(int fn,Vector v,int i){
//try{
...
if(ans==new BigNumber("1")){return "";}
if(ans.equals(new BigNumber())){return "";}
...
BigNumber booleanToBigNumber(boolean b){
	if(b){return new BigNumber("1");}
